...
$url2 = \yii\helpers\Url::to(['comisarios-auditores/contadorauditor']);
...
<?= $form->field($contador_auditor, 'natural_juridica_id')->widget(Select2::classname(), [
   ...
   <?php echo Form::widget([
    'model'=>$contador_auditor,
    'form'=>$form,
    'columns'=>2,
    'attributes'=>$contador_auditor->formAttribscontador
      ]); ?>
...
$script = <<< JS
    $('#enviar15').click(function(e){
     ...
     $('#enviar').click(function(e){
            ...
            if($('form#c_auditor').find('.has-error').length!=0){
              
                return false;
            }else
            {
                //$('form#c_auditor').submit();
                e.preventDefault();
                e.stopImmediatePropagation();
               $.ajax({
