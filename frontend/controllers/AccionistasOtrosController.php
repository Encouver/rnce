...
class AccionistasOtrosController extends BaseController
{
                         ...
                         switch ($id){
                            case 'accionista':
                                if($model->tipo_cargo!=null){
                                    ...
                                    $junta= AccionistasOtros::findOne(['documento_registrado_id' => $model->documento_registrado_id, 'junta_directiva'=>true,'natural_juridica_id'=>$model->natural_juridica_id]);
                                    if(isset($junta)){
                                         $model->tipo_cargo=$junta->tipo_cargo;
                                    } 
                                }
                    ...
                    if($model->accionista){
                        ...
                        if($model->junta_directiva){
                            $accionista= AccionistasOtros::findOne(['documento_registrado_id' => $model->documento_registrado_id, 'accionista'=>true,'natural_juridica_id'=>$model->natural_juridica_id]);
                                ...
                                $actual= AccionistasOtros::findOne($model->id);
                                $accionista= AccionistasOtros::findOne(['documento_registrado_id' => $model->documento_registrado_id, 'accionista'=>true,'tipo_cargo'=>$actual->tipo_cargo]);
                                if(isset($accionista) && $accionista->natural_juridica_id!=$model->natural_juridica_id){
                          ...
                          // $accionista= AccionistasOtros::findOne(['documento_registrado_id' => $model->documento_registrado_id, 'accionista'=>true]);
                           $accionista= AccionistasOtros::find()->where('contratista_id = :contratista and documento_registrado_id = :documento_registrado_id and  accionista = :accionista and repr_legal_vigencia is not null' , ['contratista'=>Yii::$app->user->identity->contratista_id, 'documento_registrado_id'=>$model->documento_registrado_id, 'accionista' => true])->one();
                                
                            if(isset($accionista)){
                                if($accionista->natural_juridica_id!=$model->natural_juridica_id){
    ...
    $buscar_accionista= "(natura.denominacion ILIKE "."'%" . $q ."%' or natura.rif ILIKE "."'%" . $q ."%') and natura.id=accionista.natural_juridica_id and accionista.junta_directiva=false";   
    ...
    if (!is_null($q)) {
       ...
       if(!is_null($sucursal)){
                 $buscar_accionista= "(natura.denominacion ILIKE "."'%" . $q ."%' or natura.rif ILIKE "."'%" . $q ."%') and natura.id=accionista.natural_juridica_id and accionista.accionista=true";   
            }
